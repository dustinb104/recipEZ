...
import createDataContext from "./createShoppingContext";


const blogReducer = (ShoppingState, action) => {
    switch(action.type){
        case 'get_blogposts':
        ...
        case 'delete_hero':
            return ShoppingState.filter((hero) => {
                return hero.id !== action.payload
            });
        case 'calories_hero':
            return ShoppingState.map((hero) => {
...
export const {ShoppingContext, Provider} = createDataContext(blogReducer, 
